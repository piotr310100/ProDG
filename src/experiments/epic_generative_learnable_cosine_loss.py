    ...
    for step in pbar:
        ...
        if is_U_phase:
            ...
            if config.training.lambda_diversity > 0:
                ...
                for b in range(B):
                    ch = target_channels[b].item()
                    div_channel = torch.full(
                        (K,),
                        ch,
                        device=device,
                        dtype=torch.long
                    )
                    pe_div, ppe_div, _, _ = prompt_bank(div_channel)
                    ...
                    loss_div += cosine_diversity(imgs_div)
                
                loss_div = (config.training.lambda_diversity * loss_div) / B

            total_loss = loss_purity + loss_reg + loss_entropy + loss_div
