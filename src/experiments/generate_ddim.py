...
def get_clean_real_prototypes(dataloader, positive_prototypes, channel_idx, device):
    indices = positive_prototypes[channel_idx]
    clean_images = []

    for idx in indices:
        img_tensor = dataloader.dataset[idx][0]
        # Unnormalize the ResNet-normalized tensor back to [0,1]
        img_tensor = unnormalize(img_tensor.to(device).unsqueeze(0)).squeeze(0)
        img_tensor = torch.clamp(img_tensor, 0, 1)
        img_pil = ToPILImage()(img_tensor.cpu())
        clean_images.append(img_pil)
...
def create_comparison_grid(real_data, gen_data, channel_idx):
    ...
    draw.text(
        (20, y_start_gen),
        "Generated Prototypes (Inverted & Guided)",
        fill="black",
        font=font,
    )
    curr_x = 20
...
@torch.no_grad()
def encode_to_latent(pipe, image_pil, device="cuda"):
    image = image_pil.resize((512, 512), Image.Resampling.LANCZOS)
    tensor_img = to_tensor(image).unsqueeze(0).to(device)
    tensor_img = tensor_img * 2.0 - 1.0

    latents = pipe.vae.encode(tensor_img.to(dtype=pipe.vae.dtype)).latent_dist.sample()
    latents = latents * pipe.vae.config.scaling_factor
    return latents


@torch.no_grad()
def invert(pipe, start_latents, text_embeddings, num_inference_steps, device="cuda"):
    ...
    cond_embeds = text_embeddings.chunk(2)[1]

    for i in tqdm(range(len(timesteps_list) - 1), desc="Inverting", leave=False):
        t_curr = timesteps_list[i]
        t_next = timesteps_list[i + 1]

        latents = latents.to(pipe.unet.dtype)

        noise_pred = pipe.unet(
            latents, t_curr, encoder_hidden_states=cond_embeds
        ).sample
        ...
        beta_t_next = 1 - alpha_t_next
        latents = alpha_t_next.sqrt() * pred_x0 + beta_t_next.sqrt() * noise_pred
...
def generate_and_compare(config: DictConfig):
    ...
    for channel_idx in target_channels:
        generated_data = []
        clean_real_images = get_clean_real_prototypes(
            dataloader_train, positive_prototypes_indices, channel_idx, device
        )

        for gen_idx in range(num_gen_protos):
            src_img = clean_real_images[gen_idx % len(clean_real_images)]

            start_latents = encode_to_latent(pipe, src_img, device=device)
            ...
            iterator = tqdm(
                ...
                desc=f"Ch {channel_idx} | Gen {gen_idx + 1}",
                leave=False,
            )
            ...
            for t in iterator:
                ...
                if lambda_scale > 0:
                    ...
                    scaled_x0 = 1 / pipe.vae.config.scaling_factor * pred_x0.float()
                ...
                with torch.no_grad():
                    ...
                    prev_t_idx = list(pipe.scheduler.timesteps).index(t.item()) + 1
                    if prev_t_idx < len(pipe.scheduler.timesteps):
                        prev_t = pipe.scheduler.timesteps[prev_t_idx]
                        alpha_t_prev = pipe.scheduler.alphas_cumprod[prev_t.item()]
                    else:
                    ...
                    latents = alpha_t_prev.sqrt() * pred_x0 + dir_xt
